...
def modify_order(order_id: int, items: List[Dict]) -> str:
    """Modifies the items of an existing order. If the order was already APPROVED (or COOKED),
    its inventory will be restored, and its status will be reset to DRAFT.
    ...
    if success:
        return f"Order {order_id} modified successfully. Items updated to {items}. Status reset to DRAFT."
