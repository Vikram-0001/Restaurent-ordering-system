...
def modify_order(order_id: int, items: List[Dict]) -> bool:
    ...
    try:
        # If it was APPROVED and inventory was deducted, restore it first
        if current_status == "APPROVED" and inventory_deducted == 1:
        ...
        # Modify the order items and reset status to PENDING_APPROVAL
        cursor.execute("""
            UPDATE orders
            SET items = ?, status = ?, updated_at = ?
            WHERE order_id = ?
        """, (items_json, "PENDING_APPROVAL", now_str, order_id))
        conn.commit()
        return True
    except Exception as e:
        conn.rollback()
...
def update_order_status(order_id: int, status: str, manager_note: str = "") -> bool:
    ...
    try:
        ...
        # If moving AWAY from APPROVED (e.g. to REJECTED or PENDING_APPROVAL) and inventory was deducted, restore it
        elif status != "APPROVED" and current_status == "APPROVED" and inventory_deducted == 1:
